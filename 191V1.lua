...
-- ===== INSTAN TP DENGAN PENGUNCIAN SUPER KUAT =====
function lockVehicleCompletely()
    local character = player.Character
    if not character then return end
    
    -- 1. Kunci semua part yang berpotensi jadi komponen kendaraan
    for _, child in pairs(character:GetDescendants()) do
        if child:IsA("BasePart") then
            -- Anchor semua part
            child.Anchored = true
            -- Matikan collision biar gak nabrak
            child.CanCollide = false
            -- Massa menjadi 0 biar gak kena physics
            ...
            -- CustomPhysicalProperties biar super stabil
            child.CustomPhysicalProperties = PhysicalProperties.new(0, 0, 0, 0, 0)
        end
        
        -- Khusus untuk Wheel/Vehicle parts
        if child:IsA("Part") or child:IsA("MeshPart") or child:IsA("CylinderPart") or child:IsA("WedgePart") then
            if string.find(string.lower(child.Name), "wheel") or 
               ...
               string.find(string.lower(child.Name), "velg") then
                child.Anchored = true
                child.CanCollide = false
            end
        end
    ...
    -- 2. Kunci semua joint/constraint
    for _, child in pairs(character:GetDescendants()) do
    ...
    -- 3. Matikan semua BodyMovers yang ada
    for _, child in pairs(character:GetDescendants()) do
...
function unlockVehicleCompletely()
    local character = player.Character
    if not character then return end
    ...
    -- Aktifkan kembali joint/constraint
    for _, child in pairs(character:GetDescendants()) do
        if child:IsA("JointInstance") or child:IsA("Constraint") then
            child.Enabled = true
        end
...
function instantTeleport(targetCFrame)
    ...
    LoadingStatus.Text = "MENGUNCI KENDARAAN..."
    LoadingBar.Size = UDim2.new(0.5,0,1,0)
    LoadingPercent.Text = "50%"
    task.wait(0.1)
    
    -- Kunci total semua komponen
    lockVehicleCompletely()
    
    LoadingStatus.Text = "MENTELEPORTASI INSTAN..."
    LoadingBar.Size = UDim2.new(0.8,0,1,0)
    LoadingPercent.Text = "80%"
    task.wait(0.1)
    
    -- Teleportasi instan
    hrp.CFrame = targetCFrame
    
    -- Sinkronisasi semua part
    for _, child in pairs(character:GetDescendants()) do
        if child:IsA("BasePart") and child ~= hrp then
            child.CFrame = hrp.CFrame + (child.Position - hrp.Position)
        end
    end
    
    LoadingStatus.Text = "MEMULIHKAN KENDARAAN..."
    LoadingBar.Size = UDim2.new(1,0,1,0)
    LoadingPercent.Text = "100%"
    task.wait(0.2)
    
    -- Buka kunci kembali
    unlockVehicleCompletely()
    
    LoadingFrame.Visible = false
end

-- TP Functions dengan instant teleport
...
task.wait(0.1)
TweenService:Create(Frame, tweenInfo, {Size = openSize}):Play()local player = game.Players.LocalPlayer
local UIS = game:GetService("UserInputService")
local TweenService = game:GetService("TweenService")
local VirtualInputManager = game:GetService("VirtualInputManager")
local RunService = game:GetService("RunService")
